      ...
      <div className="w-full max-w-sm bg-white rounded-2xl shadow-xl p-5">
        ...
        <div className="flex items-start justify-between mb-4">
          ...
          <button type="button" onClick={onClose}
            className="p-1 rounded-lg hover:bg-gray-100 transition-colors flex-none ml-2" aria-label="닫기">
        ...
        <div className="space-y-2 mb-5 text-sm">
          <div className="flex gap-3">
            <span className="w-20 flex-none text-xs font-medium text-gray-500 pt-0.5">기간</span>
            <span className="text-gray-800">{sub.startDate} ~ {sub.endDate}</span>
          </div>

          {/* 진행률 바 */}
          <div className="flex gap-3 items-center">
            <span className="w-20 flex-none text-xs font-medium text-gray-500">진행률</span>
            <div className="flex items-center gap-2 flex-1">
              ...
              <span className="text-xs text-gray-700 w-8 text-right">{sub.progress}%</span>
              {sub.isDelayed && (
          ...
          {sub.leader && (
            <div className="flex gap-3">
              <span className="w-20 flex-none text-xs font-medium text-gray-500 pt-0.5">담당자</span>
              <span className="text-gray-800">{sub.leader}</span>
            </div>
          )}
          {sub.description && (
            <div className="flex gap-3">
              <span className="w-20 flex-none text-xs font-medium text-gray-500 pt-0.5">설명</span>
              <span className="text-gray-800 whitespace-pre-wrap">{sub.description}</span>
            </div>
          )}
          <div className="flex gap-3">
            <span className="w-20 flex-none text-xs font-medium text-gray-500 pt-0.5">등록일</span>
            <span className="text-gray-800">{new Date(sub.createdAt).toLocaleDateString('ko-KR')}</span>
          </div>
        </div>
