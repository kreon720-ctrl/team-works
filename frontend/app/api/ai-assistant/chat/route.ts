...
const KEYWORD_STOPWORDS = new Set([
  ...
  '전체', '전부', '모두', '모든', '다',
  // 요일어 — 시점 표현이므로 view+date 로 처리. keyword 로 새면 제목에 요일 글자
  // 없는 모든 일정이 걸러져 0건 되는 사이드이펙트 차단 ('8일'·'5월' 와 동일 정책).
  // route.ts 의 resolveRelativeWeekday/resolveBareWeekday 가 날짜로 변환하므로
  // "다음주 수요일 일정 알려줘/삭제해줘" 같은 제목-없는 시점 질의가 정상 동작.
