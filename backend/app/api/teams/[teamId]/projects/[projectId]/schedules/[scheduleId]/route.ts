...
interface UpdateProjectScheduleBody {
  title?: string
...
export async function PATCH(
  request: NextRequest,
  { params }: { params: Promise<{ teamId: string; projectId: string; scheduleId: string }> }
  ...
  try {
    ...
    const existing = await getProjectScheduleById(projectId, scheduleId)
    if (!existing) {
    ...
    if (existing.created_by !== authResult.user.userId) {
      return NextResponse.json(
        { error: '프로젝트 일정 생성자만 수정할 수 있습니다.' },
        { status: 403 }
      )
    }

    const body: UpdateProjectScheduleBody = await request.json()
    ...
    const updateParams: UpdateProjectScheduleBody = {}
    ...
    if ('phaseId' in body) updateParams.phaseId = phaseId

    const updated = await updateProjectSchedule(projectId, scheduleId, updateParams)
    ...
    return NextResponse.json(toScheduleResponse(updated))
  } catch (err) {
    console.error('Update project schedule error:', err)
...
export async function DELETE(
  ...
  try {
    ...
    if (existing.created_by !== authResult.user.userId) {
      return NextResponse.json(
        { error: '프로젝트 일정 생성자만 삭제할 수 있습니다.' },
        { status: 403 }
      )
    }

    const deleted = await deleteProjectSchedule(projectId, scheduleId)

    if (!deleted) {
      return NextResponse.json({ error: '프로젝트 일정을 찾을 수 없습니다.' }, { status: 404 })
    }

    return NextResponse.json({ message: '프로젝트 일정이 삭제되었습니다.' })
  } catch (err) {
    console.error('Delete project schedule error:', err)
    return NextResponse.json({ error: '서버 내부 오류가 발생했습니다.' }, { status: 500 })
  }
}
