...
- **AI 버틀러 "찰떡"** — 4-way 자동 의도 분류 (사용법 RAG / 일반 웹검색 / 일정 조회·등록 / 거절 안내) + SSE 스트리밍 + 다중 턴 대화
...
### FR-13 AI 버틀러 "찰떡" (UC-21~25)

| ID | 요구사항 | 우선순위 | 관련 규칙 |
|----|---------|---------|-----------|
| FR-13-1 | 우측 채팅 영역 sub-탭 "AI 버틀러" — 단일 입력창 | Must | BR-16 |
| FR-13-2 | 4-way 자동 의도 분류 (`usage` / `general` / `schedule_query` / `schedule_create` / `blocked`) — 사용자가 모드 선택 안 함 | Must | BR-16 |
...
| FR-13-5 | 일정 조회(`schedule_query`) — 자연어 → view+date+keyword → backend Schedule API 직접 조회. 코드 한국어 포맷, LLM 답변 본문 0회 | Must | BR-16, BR-20 |
| FR-13-6 | 일정 등록(`schedule_create`) — 자연어 → 인자 파싱 → SSE `pending-action` confirm 카드 → 사용자 ✓ 후 INSERT | Must | BR-17, BR-18 |
| FR-13-7 | 다중 턴 — 정보 부족 시 후속 질문(`awaiting-input`), 다음 입력 결합 재요청 | Must | BR-18 |
| FR-13-8 | 거절 안내(`blocked`) — 일정 수정·삭제 / 프로젝트·채팅·공지·포스트잇·자료실 등 다른 도메인 요청은 정중한 거절 | Must | BR-19 |
...
| AI · RAG | Ollama gemma4:26b (`num_ctx 32K`, `think:false`) + nomic-embed-text 임베딩, RAG 서버(`:8787`), Open WebUI(`:8081`), SearxNG(`:8080`) — 모두 컨테이너 |
...
| AI · RAG | Ollama gemma4:26b + Open WebUI + SearxNG | docs/13·14·17 |
| 배포 | Docker Compose (단일 호스트) | nginx 8080 → backend/frontend/open-webui 라우팅 |
...
| S-05 | 팀 메인 (캘린더 + 우측 패널) | /teams/[teamId] | 팀원 | 좌측 캘린더(월/주/일/프로젝트) + 우측 영역(상단 탭: 팀채팅 / AI 버틀러) (UC-07, UC-21~25) |
| S-05A | 우측 채팅 sub-탭 (채팅 / 자료실) | (S-05 내부) | 팀원 | 채팅 ↔ 자료실 토글. 좌측 view 가 project 면 프로젝트 채팅·자료실로 자동 전환 (UC-17, UC-19) |
| S-05B | 우측 AI 버틀러 탭 | (S-05 내부) | 팀원 | 단일 입력창 + 4-way 자동 분류, SSE 스트리밍, confirm 카드, 다중 턴 (UC-21~25) |
| S-06 | 일정 상세 / 생성 / 수정 | (S-05 모달) | 팀원 | 일정 CRUD 폼 (생성자만 수정·삭제) |
