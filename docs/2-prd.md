...
| 1.8 | 2026-05-16 | - | 카카오 소셜 인증 — FR-01-7~10(OAuth+PKCE+state, email_required, 동일이메일 자동연결, password_hash nullable) 추가. 간트차트 SVG 저장 — FR-09-12 추가 |
| 1.9 | 2026-05-26 | - | docs/1 v2.3 동기화: Schedule.color/endAt nullable, 업무보고 권한 빈 배열 의미, STT 미지원 UI 동작, UC-06 명칭 정정 |
...
| P-02 | 업무 대화와 일정 맥락이 분리되어 추적 어려움 | 캘린더와 채팅을 동일 화면에서 동시 제공 |
...
- 팀 일정 조회 (월·주·일 뷰)
- 팀 일정 추가·수정·삭제 (생성은 팀 구성원 모두, 수정·삭제는 생성자 본인)
...
|  | - 일정 변경 후 관련 대화를 별도로 남기는 과정이 번거로움 |
|  | - 팀원의 이해 여부를 확인하기 어려움 |
...
| 사용 시나리오 | 공개 팀 목록에서 원하는 팀을 찾아 가입 신청 후 승인을 기다림. 승인 후 모바일로 출퇴근 중 오늘 일정과 팀 채팅을 동시에 확인하고, 업무 상황은 업무보고로 남김 |
| Pain Point | - 가입하고 싶은 팀을 찾는 방법이 없음 |
|  | - 일정과 채팅이 분리되어 맥락을 파악하기 어려움 |
|  | - 업무 상황을 구분해 보고하고 열람 범위를 통제할 공식 채널이 없음 |
...
| FR-01-7 | 카카오 소셜 로그인/회원가입 — OAuth 2.0(OIDC) Authorization Code + PKCE + state. `POST /api/auth/oauth/kakao/start` 인증 URL 발급 → `GET /api/auth/oauth/kakao/callback` code 교환·계정 매칭/생성·우리 JWT 발급 후 fragment(#)로 전달 | Must | BR-24 |
| FR-01-8 | 카카오 이메일 동의 미허락 시 가입 거절(`email_required`) 및 안내 | Must | BR-24 |
| FR-01-9 | 동일 이메일 기존 사용자에 카카오 계정 자동 연결 (중복 계정 방지) | Must | BR-24 |
| FR-01-10 | OAuth 전용 사용자는 `password_hash = NULL` 허용 (NOT NULL 제약 해제) | Must | BR-24 |

수락 조건:
- 미가입 이메일 + 유효한 비밀번호 입력 후 회원가입 요청 시 계정 생성 및 토큰 발급
- 가입된 이메일 + 올바른 비밀번호 입력 후 로그인 요청 시 토큰 발급 및 앱 진입
- 로그인·회원가입 화면 [카카오로 시작하기] → 카카오 동의 후 앱 진입(신규는 자동 가입, 기존 이메일은 자동 연결)
- 카카오 이메일 동의 거부 시 가입 거절 안내 후 로그인 화면 복귀
...
| FR-03-4 | 일정 상세 정보 조회 (title, description, color, startAt, endAt nullable) | Must | BR-01, BR-06 |
...
| FR-04-4 | endAt 입력 시 startAt < endAt 유효성 검증. endAt 미입력/null 허용 | Must | BR-02 |
| FR-04-5 | 생성자가 아닌 사용자의 수정·삭제 시도 시 403 반환 | Must | BR-02 |
| FR-04-6 | title 최대 200자, description 선택 입력, color 미입력 시 indigo 기본값 | Must | - |

수락 조건:
- 팀 구성원(LEADER·MEMBER)이 유효한 title, startAt 으로 일정 생성 시 201 Created, 팀 전체에 일정 노출
- endAt 미입력/null 은 시작시각만 있는 일정으로 저장되고, endAt 입력 시 startAt < endAt 조건만 허용
...
| FR-05-3 | 팀 구성원의 업무보고 채팅 전송 (WORK_PERFORMANCE 타입) | Must | BR-04 |
...
수락 조건:
- 팀 구성원이 type=WORK_PERFORMANCE 메시지 전송 시 채팅 이력에 저장, 팀장에게 표시
...
| FR-08-3 | 빈 배열 설정 시 MEMBER 추가 열람 권한 전체 해제. 팀장만 업무보고 열람 가능 | Must | BR-04 |
| FR-08-4 | MEMBER가 권한 설정 시도 시 403 반환 | Must | BR-04 |

수락 조건:
- 팀장이 userIds 배열 전달 시 기존 권한 전부 교체, 지정된 사용자만 업무보고 열람 가능
- 빈 배열 전달 시 MEMBER 추가 열람 권한이 모두 해제되어 팀장만 업무보고 열람 가능
...
| FR-09-12 | 프로젝트 간트차트 화면을 `[저장]` 버튼으로 SVG 파일 다운로드 (`{프로젝트명}_{연도}.svg`, 클라이언트 전용, 서버/DB 무변경) | Should | BR-25 |

수락 조건:
- 팀 구성원이 유효한 name, startDate <= endDate 조건으로 프로젝트 생성 시 201 Created
- 간트차트 화면 `[저장]` 클릭 시 현재 화면이 SVG 로 다운로드되고 다크모드 배경이 적용됨
- 생성자 본인이 수정·삭제 시 200 OK. 생성자가 아닌 사용자의 수정·삭제 시도 시 403 Forbidden
...
| FR-14-6 | 마이크 권한 거부 시 안내 토스트. 비지원 환경에서는 팀채팅 마이크 아이콘은 숨기고, AI 찰떡이 마이크 버튼은 노출 후 클릭 시 미지원 안내 표시 | Must | BR-23 |
...
- Firefox(Web Speech 비지원) + Whisper 가용 → Whisper 로 동작. Whisper 도 비가용 → 팀채팅 마이크 아이콘 비표시, AI 찰떡이 마이크 버튼 클릭 시 미지원 안내
...
| FR-15-9 | 모바일 AI 찰떡이 + 팀채팅 입력창에 마이크 버튼 제공. 팀채팅은 지원 환경에서만 렌더링, AI 찰떡이는 미지원 클릭 안내 포함 (FR-14 와 연동) | Must | BR-23 |
