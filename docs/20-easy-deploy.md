...
- 이 노트북에는 **gemma4:e4b-q4 (4-bit 양자화, ~3 GB)** 정도가 적당. 답변 속도 5~15초로 쾌적.
...
4. 적용 확인 (PowerShell):
   ```powershell
   ollama run gemma4:e4b-q4 "한 단어로 답해: hello"
   ollama ps
   ```
...
코드는 실행 시점에 Ollama 에 떠 있는 모델을 자동으로 사용합니다. `gemma4:e4b-q4` 만 띄워두면 그 모델로 동작합니다.
...
- **모델 (Models)**: Open WebUI 가 부팅·새로고침 시 Ollama 의 모델을 자동 import. 좌측 사이드바 **하단 Workspace 아이콘 → Models** 에서 `gemma4:e4b-q4`, `gemma4:e4b` 같은 본인이 받아둔 모델이 보이면 OK. 별도 프리셋(예: `gemma4-web`) 만들 필요 없음 — frontend 가 `/api/models` 조회로 자동 선택합니다.
...
2. GPU 가 비어 있으면 → `ollama run gemma4:e4b-q4 ""` 로 다시 띄우기.
...
- `ollama run gemma4:e4b-q4 ""` 로 모델 메모리에 올림.
...
**해결**:
1. `http://localhost:8081` 접속 → 좌상단 모델 드롭다운에 본인이 받아둔 모델 (`gemma4:e4b-q4` 등) 이 보이는지 확인.
2. 안 보이면 — Ollama 에 모델이 떠있는데 Open WebUI 가 sync 못 한 상태:
   ```powershell
   ollama run gemma4:e4b-q4 ""        # 모델 메모리에 로드
   docker compose restart open-webui   # Open WebUI 가 모델 list 재로드
   ```
3. 그래도 안 보이면 — `ollama list` 에 모델 자체가 없는 것. STEP 2.1 의 GGUF 다운로드 + `ollama create gemma4:e4b-q4 -f Modelfile` 절차 다시.
