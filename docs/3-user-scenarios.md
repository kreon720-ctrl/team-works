...
| SC-26B | AI 버틀러 — 일정 수정 (대상 식별 + confirm) | PA / PB (생성자) | UC-26 |
| SC-26C | AI 버틀러 — 일정 삭제 (대상 식별 + confirm) | PA / PB (생성자) | UC-27 |
| SC-27 | AI 버틀러 — 거절 안내 (일정 외 도메인) | PA~PF 공통 | UC-25 |
| SC-28 | 음성 입력(STT) — AI 찰떡이 + 팀채팅 마이크 | PA~PF 공통 | UC-28 |

### 모바일 전용 테스트 시나리오 (SC-M*)

PRD §6 반응형 UI / FR-15 모바일 최적화 UX 검증용. **viewport <640px**(모바일 emulate 또는 실기기) 에서 수행. 동일 페르소나·동일 데이터로 데스크탑(SC-01~28) 과 결과를 교차 비교해 PC 사이드이펙트가 없음을 함께 검증.

| ID | 제목 | 페르소나 | 연관 FR |
|----|------|----------|---------|
| SC-M1 | 캘린더 좌우 swipe 네비게이션 (월·주·일 이전/다음) | PB / PD / PF (모바일) | FR-15-2, FR-15-3 |
| SC-M2 | 일정 등록·수정·상세 모달 컴팩트 UI (가로폭 80%, 폰트·버튼 축소) | PB / PD | FR-15-4 |
| SC-M3 | 월간뷰 포스트잇 색상 팔레트 + 컴팩트 카드 | PB / PD | FR-15-5 |
| SC-M4 | AI 찰떡이 음성 입력으로 일정 조회·등록 (한 손, 핸즈프리) | PB / PD / PF | FR-13, FR-14, FR-15-9 |
| SC-M5 | 팀채팅 음성 입력으로 메시지 전송 | PB / PD / PF | FR-14, FR-15-9 |
| SC-M6 | 다크모드 캘린더 팝업 시인성 + 분할 화면 키보드 억제 | PB / PD | FR-15-7, FR-14-7 |
...
### 단계별 흐름 — 카카오 소셜 로그인 / 회원가입

| # | 사용자 행동 | 시스템 반응 | API |
|---|------------|-------------|-----|
| 1 | `/login` 또는 `/signup` 화면에서 [카카오로 시작하기] 버튼 클릭 | `POST /api/auth/oauth/kakao/start` 호출 (state·PKCE code_verifier 를 OAuthState 에 저장) | `POST /api/auth/oauth/kakao/start` |
| 2 | — | 응답 url(카카오 인증 페이지)로 `location` 이동 | — |
| 3 | 카카오 로그인 + 이메일·닉네임·프로필 제공 동의 | 카카오가 `redirect_uri` 로 콜백 (`?code=…&state=…`) | — |
| 4 | — | `GET /api/auth/oauth/kakao/callback` — state 검증·1회 소비 → code→token 교환 → 사용자 정보 조회 | `GET /api/auth/oauth/kakao/callback` |
| 5 | — | 계정 매칭: ① providerUserId 매칭→기존 로그인 ② 동일 이메일 User→자동 연결 ③ 신규→User 생성(`password_hash=NULL`) | — |
| 6 | — | 우리 JWT 발급 → `/auth/oauth/success#accessToken=…&refreshToken=…&user=…` 로 302 | — |
| 7 | — | success 페이지가 fragment 파싱 → 토큰·사용자 저장 → `/`(또는 redirectAfter)로 이동 | — |

> **fragment(#) 사용 이유:** querystring(?) 은 Referer·브라우저 히스토리·서버 로그에 토큰이 남을 수 있어, JS 만 읽고 서버로 전송되지 않는 fragment 로 토큰을 전달(OAuth 모범 사례).
...
| 카카오 동의 화면에서 이메일 제공 미동의 | 콜백이 `email_required` 판정 → "카카오 계정 이메일 동의가 필요합니다. 동의 후 다시 시도해주세요" 안내 후 로그인 화면 복귀 |
| 카카오 인증 세션(state) 만료·불일치 | 콜백에서 OAuthState 미발견 → "인증 세션이 만료되었거나 유효하지 않습니다. 다시 시도해주세요" 안내 |
| 사용자가 카카오 동의 거부(`error` 파라미터) | "카카오 로그인 거부" 안내 후 로그인 화면 복귀 |
...
| # | 페르소나 | 사용자 행동 | 시스템 반응 | API |
|---|----------|------------|-------------|-----|
| 1 | PA | 프로젝트 간트차트 화면 상단의 `[저장]` 버튼을 클릭한다 | `html-to-image` 가 간트 DOM 을 SVG 로 변환 (다크모드 배경색 명시 적용) | — (클라이언트 전용) |
| 2 | — | — | `{프로젝트명}_{연도}.svg` 파일이 브라우저 다운로드 폴더에 저장 | — |

> **서버/DB 무변경:** SVG 저장은 전적으로 클라이언트(브라우저)에서 처리되며 API 호출이 없다.

### 결과
- LEADER·MEMBER 모두 프로젝트를 생성할 수 있다
- 수정·삭제는 생성자 본인만 가능하다
- 프로젝트 삭제 시 하위 프로젝트 일정과 세부 일정이 함께 삭제된다
- 간트차트 화면을 SVG 파일로 내려받을 수 있다 (다크모드 배경 포함)
...
## SC-26B AI 버틀러 — 일정 수정 (대상 식별 + 새 일시 + confirm, 다중 턴)

- **페르소나**: PA (생성자) — 본인이 생성한 일정만 수정 가능
- **목표**: 자연어로 기존 일정의 시간·제목 수정 요청 → 대상 식별 → 새 일시·제목 수집 → confirm → PATCH

### 단계별 흐름 (단일 후보, 새 일시 명시)

| # | 사용자 행동 | 시스템 반응 | API |
|---|------------|-------------|-----|
| 1 | "내일 주간회의 오후 4시로 옮겨줘" 입력 | `intent: schedule_update` 분류 (`/classify`) | — |
| 2 | — | `/parse-schedule-query` 로 대상 후보 좁히기 — 키워드 "주간회의" + 날짜 "내일" → 1건 매치 | RAG `/parse-schedule-query` |
| 3 | — | `tryParseDirectDatetime("오후 4시")` 으로 새 일시 추출 (실패 시 LLM fallback 호출) | — |
| 4 | — | SSE `pending-action` (tool: `updateSchedule`, args: `{scheduleId, startAt, endAt, ...}` + teamId) → confirm 카드 | — |
| 5 | PA [✓ 승인] | `POST /api/ai-assistant/execute` → backend `PATCH /api/teams/:teamId/schedules/:scheduleId` (생성자 검증) | `POST /api/ai-assistant/execute` |
| 6 | — | 200 OK + React Query invalidate → 좌측 캘린더 자동 갱신 | — |

### 단계별 흐름 — 다중 후보 좁히기 + new-datetime turn

| # | 사용자 행동 | 시스템 반응 |
|---|------------|-------------|
| 1 | "내일 회의 옮겨줘" — 새 일시 미명시 + 키워드 "회의" 가 14:00·16:00 두 건 매치 | 후보 카드 표시 + "어떤 회의를 수정할까요? 제목이나 시각을 더 구체적으로" 안내 |
| 2 | — | SSE `sources` (후보 목록) + `awaiting-input` (`needs: 'target'`) |
| 3 | "1번" 또는 "주간회의" 입력 | 클라이언트가 직전 `updateState` 와 결합 재요청 → 대상 확정 → "새 시각을 알려주세요" (`needs: 'new-datetime'`) |
| 4 | "오후 4시" 입력 | `updateState.needs === 'new-datetime'` 분기 — `tryParseDirectDatetime` → 1~12시 AM/PM 모호 시 `awaiting-input(needs: 'time')`, 명시되면 confirm 카드 |
| 5 | 사용자 "그대로" / "유지" 발화 | `KEEP_AS_IS_RE` 매치 → 기존 startAt/endAt 유지하고 다음 단계로 |

> **multi-turn 상태**: `updateState` 객체(`needs`, `targetScheduleId`, `targetStartAt` 등) 가 클라이언트 측에서 turn 마다 carry. 서버는 stateless.

### 결과
- 사용자 ✓ 클릭 후에만 PATCH (DB 변경 0)
- 다중 후보·새 일시 누락은 후속 질문으로 채움
- 과거 시각은 60초 grace 후 거절 ("이미 지난 시각이에요. 미래 날짜·시각으로 다시 알려주세요")

### 예외 처리

| 케이스 | 시스템 반응 |
|--------|-------------|
| PB(생성자 아님) 가 PA 생성 일정 수정 시도 | backend 가 403 Forbidden, 카드에 "이 일정은 작성자만 수정할 수 있어요" 표시 |
| 후보 0건 (매치 일정 없음) | "수정할 일정을 찾지 못했어요. 제목·날짜를 다시 알려주세요" 안내 |
| bare 1~12시 (AM/PM 모호) | "오전/오후 어느 쪽일까요?" 후속 질문 — 환각 차단 |
| 사용자 [✗ 취소] | execute 호출 안 됨, "수정을 취소했어요" |

---

## SC-26C AI 버틀러 — 일정 삭제 (대상 식별 + confirm)

- **페르소나**: PA (생성자)
- **목표**: 자연어로 일정 삭제 요청 → 대상 식별 → confirm → DELETE. "취소" 동사도 삭제로 처리

### 단계별 흐름

| # | 사용자 행동 | 시스템 반응 | API |
|---|------------|-------------|-----|
| 1 | "어제 디자인 리뷰 삭제해줘" 입력 | `intent: schedule_delete` 분류 (deleteVerb "삭제" 매치) | — |
| 2 | — | `/parse-schedule-query` 로 후보 조회 (schedule_query 와 동일 메커니즘 재활용) | RAG `/parse-schedule-query` |
| 3 | — | 단일 후보 → SSE `pending-action` (tool: `deleteSchedule`, args: `{scheduleId}` + teamId) → confirm 카드 "다음 일정을 삭제할까요?" | — |
| 4 | PA [✓ 승인] | `POST /api/ai-assistant/execute` → backend `DELETE /api/teams/:teamId/schedules/:scheduleId` (생성자 검증) | — |
| 5 | — | 200 OK + 캘린더 자동 갱신 | — |
| 6 | "내일 주간회의 취소" 입력 | `intent: schedule_delete` ("취소" → DELETE_VERBS) → 위 흐름 반복 | — |

### 단계별 흐름 — 다중 후보 좁히기

| # | 사용자 행동 | 시스템 반응 |
|---|------------|-------------|
| 1 | "내일 회의 삭제해줘" (키워드 "회의" 가 14:00·16:00 두 건 매치) | SSE `sources` (후보 목록) + `token` "어떤 일정을 삭제할까요? 제목이나 시각을 더 구체적으로 알려주세요." |
| 2 | "전체 삭제" 또는 "모두 삭제" 발화 | `BULK_INTENT_RE` 감지 → "(한 번에 하나의 일정만 삭제할 수 있어요.)" 안내 동시 송출 |
| 3 | "주간회의" 또는 "14시" 입력 | 클라이언트가 직전 question 과 결합 재요청 → 단일 후보 확정 → confirm 카드 |

### 결과
- "삭제"·"제거"·"지워"·"지운"·"취소" 모두 schedule_delete 로 분류
- bulk 삭제(전체/모두/다)는 거절하고 1건씩 처리 유도
- 단일·다중 후보 처리는 SC-26B 와 유사 패턴 (단, 새 일시 입력 단계는 없음)

### 예외 처리

| 케이스 | 시스템 반응 |
|--------|-------------|
| 생성자 아닌 사용자의 삭제 시도 | 403 Forbidden, "이 일정은 작성자만 삭제할 수 있어요" |
| 후보 0건 | "삭제할 일정을 찾지 못했어요. 시점·제목을 더 구체적으로 알려주세요" |
| 후보 다중 | 좁히기 후속 질문 + 후보 sources 카드 |
| AM/PM 모호 (1~12시) | "오전/오후 어느 쪽일까요?" 후속 질문 |
| 자정 가로지르는 모호 시간대 ("야식" 등) | "기준 시각이 모호해요. '오후 6시 이후 일정' 처럼 구체적으로" |
| [✗ 취소] | execute 호출 안 됨, "삭제를 취소했어요" |

---

## SC-27 AI 버틀러 — 거절 안내 (일정 외 도메인)

- **페르소나**: PA~PF 공통
- **목표**: 프로젝트·채팅·공지·포스트잇·자료실 등 **일정 도메인 외** 요청은 정중히 거절된다. 일정 조회·등록·수정·삭제는 모두 지원(SC-25, SC-26, SC-26B, SC-26C 참조)

### 단계별 흐름

| # | 사용자 행동 | 시스템 반응 |
|---|------------|-------------|
| 1 | "프로젝트 등록해줘" 입력 | `intent: blocked, subreason: other_domain, matched: "프로젝트"` 분류 |
| 2 | — | SSE `meta` (source: "blocked") + `token` 1회 |
| 3 | — | "찰떡이는 일정 관련 작업만 도와드릴 수 있어요. 프로젝트·채팅·공지·포스트잇·자료실 같은 작업은 화면에서 직접 처리해 주세요" 안내 |
| 4 | "공지사항 작성해줘" 입력 | `intent: blocked, subreason: other_domain` (도메인 "공지사항") |
| 5 | — | 동일한 거절 안내 |
| 6 | "프로젝트 등록하는 법 알려줘" (사용법 시그널 + 거절 키워드 동시 매치) | `intent: usage` (USAGE_KEYWORDS "하는 법" 우선 매칭) |
| 7 | — | RAG 답변 — blocked 가 아닌 사용법 답변 ✓ |
| 8 | "어제 회의 삭제해줘" 입력 | `intent: schedule_delete` (BR-22) — **거절 아님**, SC-26C 흐름 진입 |

### 결과
- 일정 외 도메인 요청은 즉시 거절 (LLM 호출 0, ms 단위 응답)
- 사용법 질문은 USAGE_KEYWORDS 우선으로 RAG 답변
- 일정 CRUD (조회·등록·수정·삭제) 는 거절 아님 — 각 SC-25/26/26B/26C 로 라우팅
...
## SC-28 음성 입력 (STT) — AI 찰떡이 + 팀채팅 마이크 공통

- **페르소나**: PA~PF 공통
- **목표**: 입력창 옆 마이크 버튼으로 한국어 음성 → 텍스트 변환, 사용자 검토 후 전송

### 단계별 흐름 — 노트북 Chrome (Web Speech API)

| # | 사용자 행동 | 시스템 반응 |
|---|------------|-------------|
| 1 | AI 찰떡이 탭에서 마이크 아이콘 클릭 | `useSpeechRecognition` → UA 검사 → `webspeech` 엔진 선택 |
| 2 | — | 브라우저가 마이크 권한 요청. 사용자 허용 |
| 3 | — | `SpeechRecognition.start()` — 아이콘이 빨간색 + pulse 애니메이션 |
| 4 | PA "오늘 일정 알려줘" 발화 | interim 결과가 입력창에 라이브 표시 ("오늘…" → "오늘 일정…" → "오늘 일정 알려줘") |
| 5 | 발화 후 침묵 | 브라우저 기본 침묵 감지 → `onend` → 최종 텍스트 확정 |
| 6 | PA 검토 후 [전송] 클릭 | `handleSend` 호출, STT 세션 정리(`stt.reset`) |

### 단계별 흐름 — Galaxy 디바이스 (Whisper backend)

| # | 사용자 행동 | 시스템 반응 |
|---|------------|-------------|
| 1 | Galaxy 폰에서 마이크 클릭 | UA `SM-XXXX` 매치 → `whisper` 엔진 선택 |
| 2 | — | `getUserMedia` → MediaRecorder 녹음 시작 + AudioContext VAD (1초 침묵 감지) |
| 3 | 발화 후 1초 침묵 | VAD 트리거 → 녹음 stop → audio blob `POST /api/stt` |
| 4 | — | Whisper STT 컨테이너 (`faster_whisper`) 가 한국어 텍스트 반환 → `setTranscript` |
| 5 | — | 입력창에 텍스트 채워짐 |

### 단계별 흐름 — 팀채팅 음성 입력 (동일 hook)

| # | 사용자 행동 | 시스템 반응 |
|---|------------|-------------|
| 1 | 팀채팅 탭의 마이크 아이콘 클릭 | AI 찰떡이와 동일한 `useSpeechRecognition` 인스턴스 — 엔진 분기·발화·변환 모두 동일 |
| 2 | "오늘 회식 7시 강남역" 발화 | 텍스트가 메시지 입력창에 채워짐 |
| 3 | [전송] 클릭 | 일반 채팅 메시지로 POST. STT 잔여 정리 |

### 결과
- 노트북·iOS Safari·일반 Android Chrome → Web Speech API 로 한국어 정확도 ★★★★★
- Galaxy·Samsung Internet·Firefox → Whisper backend 로 동등한 결과
- 인식 텍스트는 자동 전송 안 함 — 오인식 수정 여지 보장
...
> **검증 환경**: viewport <640px (Chrome DevTools "Galaxy S20 Ultra" 412×900 emulate 또는 실기기). 다크모드 토글로 라이트/다크 모두 검증. 동일 데이터로 데스크탑 시나리오와 교차 비교해 **PC 사이드이펙트 0** 을 함께 확인.

---

## SC-M1 캘린더 좌우 swipe 네비게이션 (모바일)

- **페르소나**: PB (모바일 사용자)
- **목표**: 손가락 좌우 swipe 로 월/주/일 이전·다음 페이지로 부드럽게 이동

### 단계별 흐름

| # | 사용자 행동 | 시스템 반응 |
|---|------------|-------------|
| 1 | 모바일 viewport(412×900)에서 [캘린더] 탭 진입 — 월간뷰, 오늘=5월 12일 | 현재 달(5월) 표시 |
| 2 | 화면 좌→우 swipe (threshold 50px 초과) | `useSwipeGesture` 감지 → `navigateDate(-1, 'month')` → 4월로 이동, `calendar-slide-prev` keyframe(280ms cubic-bezier) 애니메이션 |
| 3 | 화면 우→좌 swipe | 6월로 이동, `calendar-slide-next` 애니메이션 |
| 4 | [주] 탭 전환 후 좌우 swipe | 주 단위로 이전·다음 이동 (동일 swipe 메커니즘) |
| 5 | [일] 탭에서 좌우 swipe | 하루씩 이동 |
| 6 | 임의 날짜 클릭 (날짜 셀) | `currentDate` 만 갱신, 슬라이드 애니메이션 **없음** (navAnimSeq 카운터 분리) |

### 결과
- 좌우 swipe 마다 ~280ms 슬라이드 인 애니메이션
- 데스크탑(≥1024px)에서는 swipe 비활성 — [이전][다음] 버튼만 사용
- 앰버 marker (orange ring) 가 currentDate 따라 이동

### 예외 처리

| 케이스 | 시스템 반응 |
|--------|-------------|
| swipe 거리 50px 미만 | 네비게이션 안 됨 (실수 방지) |
| 세로 swipe (스크롤 의도) | 네비게이션 안 됨 — 가로 거리 < 세로 거리면 무시 |

---

## SC-M2 일정 모달 컴팩트 UI (모바일)

- **페르소나**: PB / PD (모바일)
- **목표**: 일정 상세·등록·수정 모달이 모바일에서 가로폭 80%·폰트/패딩 축소·버튼 sm 사이즈로 자동 적용

### 단계별 흐름 — 상세 모달

| # | 사용자 행동 | 시스템 반응 |
|---|------------|-------------|
| 1 | 모바일에서 일정 밴드(예: "스프린트 데모") 탭 | `ScheduleDetailModal` 노출 |
| 2 | — | `useBreakpoint().isMobile=true` → Button `size="sm"`(`py-1.5 px-3 text-sm`), 헤더 X 아이콘 `w-4 h-4` |
| 3 | 본인 작성 일정이면 [수정] 버튼 노출 | 수정 클릭 → 등록 모달과 동일한 컴팩트 폼 |

### 단계별 흐름 — 등록/수정 모달

| # | 사용자 행동 | 시스템 반응 |
|---|------------|-------------|
| 1 | [일정 등록] 클릭 또는 일정 상세 → [수정] | 모달 컨테이너 `w-4/5 md:w-full max-w-md`, 내부 카드 `p-3 md:p-6` |
| 2 | — | 레이블 `text-xs md:text-sm`, 입력란 `px-2.5 py-1.5 md:px-4 md:py-2.5 text-sm md:text-base`, 색상 swatch `w-3.5 h-3.5 md:w-4 md:h-4` |
| 3 | 시작 일시 옆 캘린더 아이콘 클릭 | DateTimePicker 팝업 — 적용·닫기 버튼 `w-1/5` (가로폭 40%), 적용 버튼 `bg-amber-400` |
| 4 | 시간/분 화살표 (`^ v`) — 다크모드에서 흰색 | 시인성 OK |

### 결과
- 모바일에서 모달이 viewport 의 80% 폭, 폼 요소가 한 단계 축소
- 데스크탑 (1024px+) 에서 동일 화면 → 기존 사이즈 그대로 (md: prefix 분기)
...
- **페르소나**: PB / PD
- **목표**: 모바일 월간뷰에서 포스트잇 색상 팔레트가 자동 노출되고, 컴팩트 카드 + X 삭제 아이콘으로 표시

### 단계별 흐름

| # | 사용자 행동 | 시스템 반응 |
|---|------------|-------------|
| 1 | 모바일 캘린더 월간뷰 진입 | [월/주/일] 탭 줄 아래에 포스트잇 색상 팔레트(indigo/amber/blue/emerald/rose) 자동 노출 — 우측 정렬, 작은 크기 |
| 2 | amber 색상 클릭 → 임의 날짜 클릭 | `usePostitActions.handleDateClick` → `createPostit.mutate` → 해당 날짜에 포스트잇 등록 |
| 3 | — | PostItCard 가 모바일 분기 — 테두리·그림자 제거, 상단 색상 줄(h-2.5) 우측에 작은 X 아이콘(w-2 h-2) |
| 4 | X 아이콘 클릭 | 본인 작성이면 `onDelete` 호출 → 삭제 |
| 5 | 다른 사람이 작성한 포스트잇 | 내용 readonly, X 아이콘 없음 (수정·삭제 불가) |

### 결과
- 모바일 한 줄에 포스트잇 1개가 컴팩트하게 표시 (min-h-36px)
- PC 는 기존 카드 스타일 (테두리·그림자·휴지통 아이콘) 유지

---

## SC-M4 AI 찰떡이 음성 입력으로 일정 조회·등록 (모바일 핸즈프리)

- **페르소나**: PB / PD / PF (모바일)
- **목표**: 모바일에서 키보드 없이 마이크 한 번으로 일정 조회·등록 완결 (출퇴근·운전 시나리오)

### 단계별 흐름 — 일정 조회

| # | 사용자 행동 | 시스템 반응 |
|---|------------|-------------|
| 1 | 모바일 하단 탭 [AI 찰떡이] 클릭 | AI 어시스턴트 패널 진입 |
| 2 | 입력창 옆 마이크 아이콘 탭 | `useSpeechRecognition` 시작, 아이콘 빨간색 + pulse |
| 3 | "오늘 일정 알려줘" 발화 → 침묵 | 텍스트가 입력창에 채워짐 |
| 4 | [전송] 탭 (직접 클릭 필요) | `intent: schedule_query` → 코드 한국어 포맷 답변 즉시 표시 |

### 단계별 흐름 — 일정 등록

| # | 사용자 행동 | 시스템 반응 |
|---|------------|-------------|
| 1 | 마이크 탭 → "내일 오후 3시 팀 점심 등록해줘" 발화 | 텍스트 채워짐 |
| 2 | [전송] 탭 | `intent: schedule_create` → parse-schedule-args → confirm 카드 |
| 3 | [✓ 승인] 탭 | execute → backend POST → 캘린더 자동 갱신 |
| 4 | "13일 11시 반 직원점심" 발화 → 전송 | "X시 반" → 30분 정규화, 시간만 명시 안 됨 → "오전/오후 어느 쪽일까요?" 후속 질문 |
| 5 | 마이크 탭 → "오후" 발화 → 전송 | 11:30 PM 으로 합쳐 confirm 카드 → 승인 → 등록 완료 |

### 결과
- 키보드 호출 0회 — 모든 입력을 음성으로 완결
- "점심" 같은 식사 키워드는 keyword 매치 (시간대 band 아님)
- 다중 턴 후속 질문도 음성으로 응답 가능

### 예외 처리

| 케이스 | 시스템 반응 |
|--------|-------------|
| 마이크 권한 거부 | 토스트 안내, 텍스트 키보드 입력으로 우회 가능 |
| 인식 결과가 틀림 | 사용자가 입력창에서 텍스트 수정 후 [전송] (자동 전송 안 함) |
...
- **페르소나**: PB / PD / PF
- **목표**: 팀채팅 입력창의 마이크 버튼으로 빠른 메시지 전송 (이동 중·운전 중)

### 단계별 흐름

| # | 사용자 행동 | 시스템 반응 |
|---|------------|-------------|
| 1 | 모바일 [팀채팅] 탭 진입 | 채팅 목록 + 입력창 표시 |
| 2 | 입력창 옆 마이크 아이콘 탭 | STT 시작 (AI 찰떡이와 동일 hook) |
| 3 | "오늘 회식 강남역 7시 모입니다" 발화 → 침묵 | 텍스트 채워짐 |
| 4 | [전송] 클릭 | NORMAL 타입 메시지 전송, `stt.reset()` 호출 — STT 잔여 세션 정리 |
| 5 | 메시지 전송 직후 | 입력창 자동 포커스 해제 → 시스템 키보드 다시 안 올라옴 |

### 결과
- 운전·이동 중에도 한 손으로 메시지 전송 가능
- 전송 후 키보드가 재호출되지 않아 다음 화면 가림 방지

---

## SC-M6 다크모드 캘린더 시인성 + 분할 화면 키보드 억제

- **페르소나**: PB / PD
- **목표**: 다크모드에서 캘린더 팝업의 월 표시 색상이 명확하고, AI 찰떡이 분할 화면에서 키보드가 화면을 가리지 않는다

### 단계별 흐름 — 다크모드 시인성

| # | 사용자 행동 | 시스템 반응 |
|---|------------|-------------|
| 1 | 헤더의 다크모드 토글 활성화 | 전체 UI 다크 테마 적용 |
| 2 | 일정 등록 모달 → 시작 일시 → 캘린더 아이콘 | DateTimePicker 팝업 노출 |
| 3 | 팝업 내 날짜 셀 색상 검증 | 이전 달(4월) 회색(text-gray-500), 이번 달(5월) 흰색, 오늘(12일) 앰버골드(text-amber-400) |
| 4 | 월 이동 화살표 (`< >`) 색상 | 흰색 (다크모드에서 검정으로 보이던 문제 해결) |
| 5 | 시간/분 위·아래 화살표 (`^ v`) 색상 | 흰색 |
| 6 | [적용] 버튼 색상 | bg-amber-400 (앰버골드) |

### 단계별 흐름 — 분할 화면 키보드 억제

| # | 사용자 행동 | 시스템 반응 |
|---|------------|-------------|
| 1 | 모바일 [AI 찰떡이] 탭 진입 | composer 영역 표시 |
| 2 | composer 의 캘린더 아이콘 클릭 | 화면이 상·하 분할 — 상단 캘린더 미리보기, 하단 AI 입력창 |
| 3 | 입력창에 자동 포커스 가지 않음 | 시스템 키보드 표시 안 됨 (`inputRef.current?.blur()` 강제) |
| 4 | 마이크 아이콘 탭 → 음성 입력 | 인식 결과 텍스트 채워짐, 여전히 포커스 안 감 |
| 5 | [전송] 클릭 후 AI 응답 출력 | 응답 완료 후에도 입력창 포커스 안 가짐 — 키보드 안 올라옴 |
| 6 | 사용자가 직접 입력창 탭 | 그제서야 포커스 + 키보드 (직접 타자 의도) |

### 결과
- 다크모드 캘린더 팝업의 모든 텍스트·아이콘이 명도 차이로 시인성 확보
- 분할 화면에서 음성 입력 흐름 동안 키보드가 화면 절반 가림 방지

### 예외 처리

| 케이스 | 시스템 반응 |
|--------|-------------|
| 사용자가 입력창 직접 탭 | 포커스 + 키보드 — 직접 타자 입력 정상 |
...
| SC-23~28, SC-M4·M5 | POST | `/api/ai-assistant/chat` | AI 어시스턴트 6-way 분기 SSE (frontend route) |
| SC-26, SC-26B, SC-26C | POST | `/api/ai-assistant/execute` | confirm 카드 ✓ 후 도구 실행. TOOL_WHITELIST = `{createSchedule, updateSchedule, deleteSchedule}` |
| SC-23, SC-25, SC-26B/C, SC-27 | POST | (RAG `:8787`) `/classify` | 의도 분류기 — 6-way 키워드 매치 + LLM 위험 분기 검증 |
| SC-25, SC-26B, SC-26C | POST | (RAG `:8787`) `/parse-schedule-query` | 자연어 → view+date+keyword (식사 단어는 keyword). schedule_update/delete 의 대상 식별에도 재활용 |
| SC-26 | POST | (RAG `:8787`) `/parse-schedule-args` | 자연어 → 일정 등록 인자 (X시 반 → 30분 정규화, 또는 후속 질문) |
| SC-23 | POST | (RAG `:8787`) `/chat` | RAG 답변 stream (think:false) |
| SC-28, SC-M4·M5 | POST | `/api/stt` | Whisper STT 변환 (audio blob → 한국어 텍스트). Web Speech 분기는 브라우저 내장 — API 호출 없음 |

> **schedule_update / schedule_delete 의 대상·새값 파싱 위치**: 별도 RAG 엔드포인트 없음. frontend `chat/route.ts` 가 `parse-schedule-query` 로 후보 좁히기 + 자체 `tryParseDirectDatetime` + `KEEP_AS_IS_RE` 패턴 + Open WebUI LLM fallback 으로 직접 처리. multi-turn 상태(`updateState`/`deleteState`) 는 클라이언트 측이 turn 마다 carry.
